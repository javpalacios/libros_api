...
async def create_autor(autor: AutorCreate, service: AutorService = Depends()):
    return service.create(autor)

@router.get("/", response_model=list[AutorResponse])
async def read_autores(service: AutorService = Depends()):
    return service.get_all()

@router.get("/{id}", response_model=AutorResponse)
async def read_autor(id: int, service: AutorService = Depends()):
    return service.get_by_id(id)

@router.patch("/{id}", response_model=AutorResponse)
async def update_autor(id: int, autor_data: AutorUpdate, service: AutorService = Depends()):
